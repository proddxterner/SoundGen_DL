...
from preprocess import min_max_normaliser
import librosa
...
class SoundGenerator:
    ...
    def __init__(self, vae, hop_length):
        ...
        self.min_max_normaliser = min_max_normaliser(0, 1)
    ...
    def convert_spectrograms_to_audio(self, spectrograms, min_max_values):
        ...
        for spectrogram, min_max_value in zip(spectrograms, min_max_values):
            ...
            # denormalising 
            denorm_log_spec = self.min_max_normaliser.denormalize(log_spectrogram, min_max_value["min"], min_max_value["max"])
            # log spectrogram to linear spectrogram - from dB to normal amplitude
            spec = librosa.db_to_amplitude(denorm_log_spec)
            # apply inverse short time fourier transform
            signal = librosa.istft(spec, self.hop_length)
